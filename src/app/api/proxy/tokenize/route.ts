 ...
 * Deploy a token for the user's proxy via Clanker.
 */
export async function POST(request: Request) {
  ...
  if (!proxy) return NextResponse.json({ error: "No proxy found" }, { status: 404 });

  if (proxy.tokenAddress) {
    return NextResponse.json({ error: "Token already deployed", tokenAddress: proxy.tokenAddress }, { status: 409 });
  }
